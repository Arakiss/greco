...
- The loop is now closed end to end (`loop run --with-solver` gates apply on a measured marginal improvement), but the *experiment* is not yet answered. With the current always-pass eval suite the measured delta is zero, so the loop honestly applies nothing. Confirming the thesis needs operator eval tasks a procedure can actually move, plus evidence that proposed procedures move them. That open question is the point — it is now runnable, not a code gap.
