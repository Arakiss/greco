...
The experiment is now running for real. A five-task evaluation suite derived from measured operator-friction categories replaced the always-pass placeholder (WS1), and the first baseline (WS2, three solver runs per task) produced the project's first empirical result — an uncomfortable, useful one: **only one of five tasks exposes friction a modification could move**. The frontier solver already reads the conventions file, uses the index, stages explicit paths, and consults the troubleshooting doc unprompted at this fixture scale. See *First empirical result* below; the WS3 fork it forces is the current decision point.
...
- `greco modification compare`: runs the solver on every task with and without the candidate procedure active and reports the real `primary_improvement_ppm` delta — the marginal effect the deterministic criteria could never see.
- `greco loop run --with-solver`: the autonomous loop runs that comparison during validation and will only apply a modification that shows a positive measured marginal improvement. The gate can only make admission stricter, never looser.

Not yet — and called out wherever it matters below:

- The loop is closed end to end (`loop run --with-solver` gates apply on a measured marginal improvement) AND the suite is no longer a placeholder: five fixtures under `fixtures/eval-suite/` with deterministic criteria (WS1), plus a three-run baseline per task under a clean `GRECO_HOME` (WS2, `docs/operations/ws2-baseline.md`). What is not yet answered is the experiment itself, and WS2 sharpened the question: with only `t3-edit-discipline` admitted as movable, the thesis can currently be tested on one friction cluster. The other four fixtures showed zero target friction — a finding about fixture design and frontier-solver behavior, recorded rather than hidden.
...
## First empirical result (2026-07-05)

WS2 baseline: 15 solve runs (3 per task, `gpt-5.4`, clean harness state, estimated $0.28 total). Objective success was 15/15. Movability admission per the pre-registered rule (non-zero profiled friction, variance tight enough for 5% delta detection):

| Task | Profiled friction observed | Admitted |
|---|---|---|
| t1-conventions | adherence misses 0/0/0 — solver read `CONVENTIONS.md` first, every run | No |
| t2-read-economy | generated-file reads 0/0/0 — solver used `INDEX.md` unprompted | No |
| t3-edit-discipline | retracements 1/1/1, deterministic | **Yes** |
| t4-staging | broad staging 0/0/0 — explicit path staging, every run | No |
| t5-recovery | target counters zero; incidental retracements too noisy | No |

The reading: the movability hypotheses were calibrated against friction patterns frontier solvers no longer exhibit at direct-fixture scale. That is a real result about where harness modifications can and cannot pay — not a failure of the machinery, which behaved exactly as designed (the admission rule refused four tasks instead of flattering them). It forces the WS3 fork documented in the roadmap: run the narrow experiment on `t3` end to end, and/or harden fixtures with re-registered hypotheses before any re-baseline.
...
**The load-bearing gap has moved from code to evidence.** The solver-in-validation wiring is closed (`loop run --with-solver` measures the real marginal delta and gates apply on it), and the suite is real. What limits the experiment today is suite movability: one admitted friction cluster out of five (see *First empirical result*). The Phase 3 verdict can now come from real data — but from a narrow base. The two honest ways forward, and the rule that keeps them honest, live in the roadmap's WS3 fork: any hardened fixture gets its movability hypothesis re-registered in writing BEFORE re-baselining, so the experiment cannot quietly tune its tasks until the answer looks good.

## Status Summary

The alpha skill cycle is closed and documented. The recalibrated v0 begins at `0.4.0-alpha.1` with instrumentation and baseline (Phase 1). `0.5.0-alpha.1` is the first release-gated Phase 2 alpha because it adds new CLI commands and a persistent modification-manifest format. `0.6.0-alpha.1` is the first release-gated Phase 3 alpha because it adds autonomous loop commands and persistent budget/freeze/checkpoint state. `0.6.1-alpha.1` is a Phase 3 patch because loop decisions now carry baseline-vs-candidate comparison deltas and apply only when the threshold policy admits them. `0.7.0-alpha.1` is a minor alpha because it adds `greco loop gate`, the public acceptance verdict surface for Phase 3 windows. `0.8.0-alpha.1` is a minor alpha because it adds `greco eval solve`, which runs the model inside an isolated workspace snapshot and grades it against the operator's criteria — the first half of putting the solver inside validation. `0.9.0-alpha.1` is a minor alpha because it adds `greco modification compare`, which measures a candidate's marginal `primary_improvement_ppm` by solving every task with and without the procedure active — the measurement the Phase 3 gate needs. `0.10.0-alpha.1` is a minor alpha because it adds `greco loop run --with-solver`, which gates autonomous apply on a positive measured marginal improvement — closing the solver-in-validation loop end to end. Current unversioned work on `main`: the movable evaluation suite (WS1 — five fixtures under `fixtures/eval-suite/`, versioned-suite loading in `greco eval`) and the first baseline with movability admission (WS2 — `docs/operations/ws2-baseline.md`, 1/5 admitted). The tracked documentation set in `docs/` is the public product contract.
