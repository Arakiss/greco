...
- The *autonomous loop* (`greco loop run`) does not yet call this measurement — `greco modification compare` proves a candidate's effect on demand, but the loop still admits on the deterministic comparison, so its `primary_improvement` reads `0ppm` and the Phase 3 gate cannot return `Pass` from a loop run. Wiring `modification compare` into the loop and gating autonomous `--apply` on a Phase 3 `Pass` is the remaining step.
